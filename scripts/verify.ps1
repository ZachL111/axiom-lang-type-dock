...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-axiom-lang-type-detail.ps1

julia --project=. tests/domain_review.jl

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-axiom-lang-type-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-axiom-lang-type-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-axiom_lang_type_dock-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
